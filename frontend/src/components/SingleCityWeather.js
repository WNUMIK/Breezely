    ...
    const fetchWeatherData = async () => {
      if (currentCity) {
        ...
        try {
          ...
          // Check if coordinates are present in response
    ...
    <div className="dashboard">
      <div className="header-buttons-row">
        ...
        <div className="input-section">
          <input type="text" placeholder="Enter city" value={currentCity} onChange={(e) => setCurrentCity(e.target.value)} />
          <button className="input-section-button" onClick={handleSearch}>Search</button>
        </div>
        <button className="favorite-button" onClick={saveFavoriteCity} style={{ backgroundColor: favoriteButtonColor }}>
          {favoriteButtonText}
        </button>
      </div>
      ...
      {weatherData && (
        <div className="weather-dashboard">
          ...
          <div className="weather-animation card">
            <Lottie loop animationData={getWeatherAnimation(weatherData.description)} play style={{ width: '150px', height: '150px' }} />
          </div>
